...
function NAGangplankMenu()
	Menu.NewTree("NAGangplankCombo", "Combo", function ()
		...
		Menu.Slider("Combo.CastEHC", "E Hit Chance", 0.60, 0.05, 1, 0.01)
...
local function BarrelsIntersect(posE1, posE2)
	...
	vector1 ,vector2 = Geometry.CircleCircleIntersection(posE1, spells.E.BarrelRadius,
			posE2, spells.E.BarrelRadius)
...
local function CanKillBarrel(barrel)
	...
	local delay = Game.GetLatency()/1000 + spells.Q.Delay
	local flightTime = delay + (barrel.Position:Distance(Player.Position)/spells.Q.Speed)

	if barrel.Health == 2 and os.clock() - timer[barrel] < exptime then
		if os.clock() - timer[barrel]  > (exptime - flightTime)  then 
			return true
		end
	end

	if barrel.Health == 2 and os.clock() - timer[barrel] >= exptime then
		if os.clock() - timer[barrel]  > (exptime * 2 - flightTime)  then 
			return true
		end
	end
...
local function GetNearestBarrelToEnemy(target)
	...
	for handle, barrel in pairs(barrels) do
		...
		if nearBarrel then
			...
			local distBarrel = enemyPos:Distance(barrel.Position)
			local distNearBarrel = enemyPos:Distance(nearBarrel.Position)
		...
		if checkBarrel then
			...
			if nearBarrel then
				if CanKillBarrel(nearBarrel) then
					if Player.Position:Distance(nearBarrel) <= 170 and Player.CanAttack then
						Input.Attack(nearBarrel)
					else
						spells.Q:Cast(nearBarrel)
					end
				end
...
pred = Prediction.GetPredictedPosition(target, spells.E, Player.Position)
	if spells.E:IsReady()  then

		local dist = Player:Distance(target.Position)

		local barrelTarget = GetNearestBarrel()
		local farBarrelTarget = GetNearestBarrelToEnemy(target)
		
		
			
			if GetBarrelsAmmo() >= 1 and CountBarrelsNearMe() >= 1 and  target.Position:Distance(farBarrelTarget.Position) < spells.E.BarrelRadius*3 - 150 then

				
				if CountBarrelsNearMe() == 1 then
					
					if barrelTarget and CanKillBarrel(barrelTarget) then
						
							--[[local barrelDist = barrelTarget.Position:Extended(target.Position,(spells.E.BarrelRadius*2-50))

							

								predinput = Spell.Skillshot({
										Slot = Enums.SpellSlots.E,
										Range = spells.E.BarrelRadius * 2 ,
							...
							if pred and pred.HitChance >=hitChance then
								if pred.CastPosition:Distance(barrelTarget.Position) > spells.E.BarrelRadius*2 then 
										
											local substract =  pred.CastPosition:Distance(barrelTarget.Position) - spells.E.BarrelRadius * 2 + 50
											local castPos = pred.CastPosition:Extended(barrelTarget.Position,substract)


											--print( "resultcastPos" ..substract)

							

											if castPos:Distance(barrelTarget.Position) <= spells.E.BarrelRadius * 2+50 then 
												spells.E:Cast(castPos)   
												--spells.E:Cast(barrelDist)
					...
					if CountBarrelsNearMe() > 1 then
						...
						if barrelTarget and farBarrelTarget and CanKillBarrel(barrelTarget) then
							...
							if pred and pred.HitChance >=hitChance then
								...
								if pred.CastPosition:Distance(barrelTarget.Position) < pred.CastPosition:Distance(farBarrelTarget.Position) then 
									...
									if pred.CastPosition:Distance(farBarrelTarget.Position) > spells.E.BarrelRadius*2  then 
										
											local substract =  pred.CastPosition:Distance(farBarrelTarget.Position) - spells.E.BarrelRadius * 2 + 50
											local castPos = pred.CastPosition:Extended(farBarrelTarget.Position,substract)

											

										
											if castPos:Distance(farBarrelTarget.Position) <= spells.E.BarrelRadius * 2+50 then 
												spells.E:Cast(castPos)   
												--spells.E:Cast(barrelDist)
...
local function AutoWCC()
	...
	if IsTargetCC(Player) and not Player.IsTeleporting then
		CastW()
	end
...
local function OnHighPriority()
	...
	for handle, barrel in pairs(barrels) do
		...
		if (barrel.Health == 2) and timer[barrel] == nil then
			print(os.clock())
			timer[barrel] = os.clock()
		end
...
local function OnDraw()
	...
	if Player:GetSpell(SpellSlots.E).IsLearned and Menu.Get("Drawing.DrawE") then
		...
		Renderer.DrawCircle3D(Renderer.GetMousePos(), spells.E.BarrelRadius, 30, 1.0, Menu.Get("Drawing.DrawEColor"))
