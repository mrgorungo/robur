--[[
  ____  _                         _  __                       _ _       
 / ___|| |_ ___  _ __ _ __ ___   | |/ /__ _ ___ ___  __ _  __| (_)_ __  
 \___ \| __/ _ \| '__| '_ ` _ \  | ' // _` / __/ __|/ _` |/ _` | | '_ \ 
  ___) | || (_) | |  | | | | | | | . \ (_| \__ \__ \ (_| | (_| | | | | |
 |____/ \__\___/|_|  |_| |_| |_| |_|\_\__,_|___/___/\__,_|\__,_|_|_| |_|
                                                                                                                                                               
]]
if Player.CharName ~= "Kassadin" then return end
--[[ require ]]
require("common.log")
module("Storm Kassadin", package.seeall, log.setup)
clean.module("Storm Kassadin", clean.seeall, log.setup)
--[[ SDK ]]
local SDK         = _G.CoreEx
...
local function GetRStacks()
    local buff = Player:GetBuff("RiftWalk")
    if buff then
        return buff.Count
    else 
        return 0
...
local function FlashR(spell,obj)
    local Castpos = Player.Position:Extended(obj.Position, obj:Distance(Player))
...
local function dmg(spell)
    local dmg = 0
    local Extra = (40 + (R:GetLevel() - 1) * 10) + (0.1 * Player.TotalAP) + Player.MaxMana * 0.01
    if spell.Key == "Q" then 
        dmg = (65 + (Q:GetLevel() - 1) * 30) + (0.7 * Player.TotalAP)
    end
    if spell.Key == "E" then 
        dmg = (80 + (E:GetLevel() - 1) * 25) + (0.8 * Player.TotalAP)
    end
    if spell.Key == "R" then
        dmg = (80 + (R:GetLevel() - 1) * 20) + (0.4 * Player.TotalAP) + Player.MaxMana * 0.02 + (Extra * GetRStacks())
    end
...
local function Wdmg()
    local dmg = 0
    local Extra = 0
    local passive = 20 + (0.1 * Player.TotalAP)
    local time = os.clock()
...
local function TotalDmg(t)
    ...
    if Q:IsReady() then
        Damage = Damage + DmgLib.CalculateMagicalDamage(Player,t,dmg(Q))
    end
    if W:IsReady() then
        Damage = Damage + DmgLib.CalculateMagicalDamage(Player,t,Wdmg()) + DmgLib.CalculatePhysicalDamage(Player,t,dmgsheen())
    end
    if E:IsReady() then
...
function Kassadin.OnPreAttack(args)
    ...
    if mode == "Harass" and Target.IsHero and Menu.Get("ManaSlider") < Player.ManaPercent * 100 then 
...
function  KassadinNP.Combo()
    ...
    if CanCast(Q,mode) then 
        for k,v in pairs(GetTargets(Q)) do 
            if v:Distance(Player) < Player.AttackRange + 50 and W:IsReady() then return end
...
function KassadinHP.Waveclear()
    if Menu.Get("ManaSliderLane") < Player.ManaPercent * 100 and Menu.Get("SpellFarm") then 
        if Lane(E) then 
            local EPoint = {}
            for k, v in pairs(Obj.Get("enemy", "minions")) do
                if ValidAI(v,E.Range) then
                    ...
                    local isKillable = DmgLib.CalculateMagicalDamage(Player, minion, dmg(E)) > minion.Health
                    if pos:Distance(Player.Position) < E.Range then
                        table.insert(EPoint, pos)
                    end
                end                       
            end
            local bestPos, hitCount =  Geo.BestCoveringCone(EPoint,Player.Position, 60)
        ...
        if Lane(R) and GetRStacks() <= Menu.Get("Lane.RS") and not E:IsReady() then 
            ...
            for k, v in pairs(Obj.Get("enemy", "minions")) do
                if ValidAI(v,R.Range) then
                    ...
                    if pos:Distance(Player.Position) < R.Range then
...
function KassadinNP.Waveclear()
    if Menu.Get("ManaSliderLane") < Player.ManaPercent * 100 and Menu.Get("SpellFarm") then 
        if Lane(W) then 
            for k,v in pairs(Obj.Get("enemy","minions")) do
                local minion = v.AsAI
                local trueRange = W.Range + Player.BoundingRadius + minion.BoundingRadius
                if ValidAI(v,trueRange) then 
                    local healthPred = W:GetHealthPred(minion)
                    local WDmg = DmgLib.CalculateMagicalDamage(Player, minion, Wdmg())
                    local AADmg = math.floor(Player.TotalAD)
    ...
    if LastHit(R) and GetRStacks() <= Menu.Get("LastHit.RS") and not E:IsReady() then 
        ...
        for k, v in pairs(Obj.Get("enemy", "minions")) do
            if ValidAI(v,R.Range) then
                local minion = v.AsAI
                local pos = minion:FastPrediction(Game.GetLatency()+ R.Delay)
                local isKillable = DmgLib.CalculateMagicalDamage(Player, minion, dmg(R)) > minion.Health
                if pos:Distance(Player.Position) < R.Range and isKillable then
                    table.insert(RPoint, pos)
                end
            end                       
        end
...
function KassadinNP.Flee()
    if Flee(R) then
        local CastPos = Player.Position:Extended(Renderer.GetMousePos(), R.Range)
...
function Kassadin.LoadMenu()
    Menu.RegisterMenu("StormKassadin", "Storm Kassadin", function()
    ...
    Menu.RegisterPermashow("KassadinPermaShow", "          Storm Kassadin : Toggle Keys           ", function()
