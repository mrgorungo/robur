...
function Utils.IsRareMinion(Target)
  ...
  if
    Target.IsDragon or
    Target.IsBaron
    -- Target.CharName == "SRU_Baron" or
    -- Target.CharName == "SRU_RiftHerald" or
    -- Target.CharName == "SRU_Dragon_Air" or
    -- Target.CharName == "SRU_Dragon_Fire" or
    -- Target.CharName == "SRU_Dragon_Earth" or
    -- Target.CharName == "SRU_Dragon_Water" or
    -- Target.CharName == "SRU_Dragon_Elder"
  then
    return true
  end
...
function Jinx.Logic.Q(Target, EnableOutOfRange, EnableMinHit, MinHitCount)
  ...
  local InRange = Utils.IsInRange(LocalPlayer.Position, Target.Position, MiniGunRange, math.huge)
...
function Jinx.Logic.W(Target, Hitchance, Enable, EnableManualRange, MinRange)
  ...
  if
    (not EnableManualRange and Utils.IsInRange(LocalPlayer.Position, Target.Position, BazookaRange, math.huge)) or
    (EnableManualRange and Utils.IsInRange(LocalPlayer.Position, Target.Position, MinRange, math.huge))
  then
...
function Jinx.Logic.R(Target, Hitchance, TravelTime, Enable, MinRange, MaxRange)
  ...
  local HealthPredicted = HealthPrediction.GetHealthPrediction(Target, TravelTime, false)

  if
    Utils.IsInRange(LocalPlayer.Position, Target.Position, MinRange, MaxRange) and
    Jinx.R.GetDamage(Target, false) >= HealthPredicted
  then
...
function Jinx.Logic.RKS(Target, TravelTime, Enable)
  ...
  local RareMinionInRange = Utils.TargetsInRange(Target, 400, "neutral", "minions", Utils.IsRareMinion)
  ...
  local HealthPredicted = HealthPrediction.GetHealthPrediction(Minion, TravelTime, false)
  
  if Jinx.R.GetDamage(Minion, true) >= HealthPredicted then
...
function Jinx.Logic.Combo()
  ...
  Target = Jinx.Q:GetTarget()
  if Jinx.Logic.Q(Target, Menu.Get("Combo.Q.OutOfRange"), Menu.Get("Combo.Q.MinHit"), Menu.Get("Combo.Q.MinHitCount")) then return true end

  Target = Jinx.W:GetTarget()
  if Jinx.Logic.W( Target, Menu.Get("Combo.W.HitChance"), Menu.Get("Combo.W.Use"), Menu.Get("Combo.W.ManualRange"), Menu.Get("Combo.W.MinRange")) then return true end

  Target = Jinx.E:GetTarget()
  if Jinx.Logic.E(Target, Menu.Get("Combo.E.HitChance"), Menu.Get("Combo.E.Use")) then return true end
...
function Jinx.Logic.Harass()
  ...
  Target = Jinx.Q:GetTarget()
  if Jinx.Logic.Q(Target, Menu.Get("Harass.Q.OutOfRange"), Menu.Get("Harass.Q.MinHit"), Menu.Get("Harass.Q.MinHitCount")) then return true end

  Target = Jinx.W:GetTarget()
  if Jinx.Logic.W(Target, Menu.Get("Harass.W.HitChance"), Menu.Get("Harass.W.Use"), Menu.Get("Harass.W.ManualRange"), Menu.Get("Harass.W.MinRange")) then return true end
  ...
  for _, Target in pairs(Targets) do
    if Target then
      ...
      if Utils.IsValidTarget(Target) then
        ...
        if Jinx.Logic.R(Target, Menu.Get("Auto.R.HitChance"), TravelTime, Menu.Get("Auto.R.Use"), Menu.Get("Auto.R.MinRange"), Menu.Get("Auto.R.MaxRange")) then return true end
...
function Jinx.LoadMenu()
  Menu.RegisterMenu("KappaJinx", "KappaJinx", function ()
    Menu.ColumnLayout("Casting", "Casting", 2, true, function ()
      ...
      Menu.Checkbox("Combo.Q.MinHit", "When Can Hit", true)
      ...
      Menu.Checkbox("Combo.W.ManualRange", "Manual Range", false)
      Menu.Slider("Combo.W.MinRange", "Min Range", 0, 0, Jinx.W.Range, 10)
      Menu.ColoredText("> E", 0x0066CCFF, false)
      Menu.Checkbox("Combo.E.Use", "Use", true)
      Menu.Dropdown("Combo.E.HitChance", "HitChance", 5, HitChanceStrings)
      Menu.NextColumn()
      ...
      Menu.Dropdown("Harass.W.HitChance", "HitChance", 5, HitChanceStrings)
      Menu.Checkbox("Harass.W.ManualRange", "Manual Range", false)
      Menu.Slider("Harass.W.MinRange", "Min Range", 0, 0, Jinx.W.Range, 10)
    end)
    Menu.Separator()
    Menu.ColumnLayout("Drawings", "Drawings", 2, true, function ()
      ...
      Menu.Slider("Auto.R.MinRange", "Min Range", Jinx.Q.Range, 0, 3000, 100)
      Menu.Slider("Auto.R.MaxRange", "Max Range", 4000, 3000, 50000, 100)
      Menu.Dropdown("Auto.R.HitChance", "HitChance", 5, HitChanceStrings)
...
function Jinx.OnGapclose(Target, DashInstance)
  if not Menu.Get("Auto.E.Gapclose") then return false end

  Target = Target.AsHero

  if Target.IsAlly then return false end
  if not Target.IsHero then return false end

  return Jinx.E:CastOnHitChance(Target, HitChance.Dashing)
end
