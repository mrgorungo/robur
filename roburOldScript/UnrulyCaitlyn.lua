...
function Caitlyn.LoadMenu()
    Menu.RegisterMenu("UnrulyAshe", "Unruly Caitlyn", function()
        Menu.ColumnLayout("cols", "cols", 2, true, function()
            Menu.ColoredText("Combo", 0xFFD700FF, true)
            Menu.Checkbox("Combo.UseQ",  "Use [Q]", true)
            Menu.Slider("Combo.ChanceQ", "HitChance [Q]", 0.75, 0, 1, 0.05)
            Menu.Slider("Combo.SafeQ",   "Block [Q] Enemies Nearby", 1, 0, 5)
            Menu.Checkbox("Combo.UseW",  "Use [W]", true)
            Menu.Slider("Combo.ChanceW", "HitChance [W]", 0.85, 0, 1, 0.05)
            Menu.Slider("Combo.SaveW",   "Save Min X Traps", 1, 0, 5)
            ...
            Menu.Slider("Harass.SaveW",   "Save Min X Traps", 2, 0, 5)
        ...
        Menu.Slider("Misc.SafeR",   "Block [R] Enemies Nearby", 1, 0, 5) 
...
function Caitlyn.Auto()
    ...
    if Menu.Get("Misc.AutoR") and Spells.R:IsReady() and CountHeroesInRange(pPos, pRange, "enemy") <= Menu.Get("Misc.SafeR") then
        local rawDmg = Caitlyn.GetRawDamageR()
        for k, hero in ipairs(Spells.R:GetTargets()) do
            local dist = hero:Distance(pPos)
            if dist > pRange and CountHeroesInRange(hero.Position, 500, "ally") == 0 then
                local timeToReach = Spells.R.Delay + dist/Spells.R.Speed
                local dmg = DamageLib.CalculatePhysicalDamage(Player, hero, rawDmg)
                if dmg > HealthPred.GetKillstealHealth(hero, timeToReach, Enums.DamageTypes.Physical) then
                    if Spells.R:Cast(hero) then
                        return true
                    end
                end
            end
...
function Caitlyn.ComboLogic(mode)
    ...
    if Caitlyn.IsEnabledAndReady("W", mode) and Spells.W:GetCurrentAmmo() > Menu.Get(mode..".SaveW") then
        local minChance = Menu.Get(mode..".ChanceW")
        for k, hero in ipairs(Spells.W:GetTargets()) do
            if not blockList[hero.Handle] and Spells.W:CastOnHitChance(hero, minChance) then
    ...
    if Caitlyn.IsEnabledAndReady("Q", mode) and enemiesAround <= Menu.Get(mode..".SafeQ") then
        local minChance = Menu.Get(mode..".ChanceQ")
        for k, hero in ipairs(Spells.Q:GetTargets()) do
            if Spells.Q:CastOnHitChance(hero, minChance) then
                return
            end
        end
    end    
