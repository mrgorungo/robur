...
require("common.log")
module("ShulepinSamira", package.seeall, log.setup)
clean.module("ShulepinSamira", clean.seeall, log.setup)

local VERSION = "1.0"
local LAST_UPDATE = "08/11/2020"
...
Menu.RegisterMenu("SSamira", "Shulepin Samira", function()
    ...
    Menu.NewTree("Q", "[Q] Flair", function()
    ...
    Menu.NewTree("W", "[W] Blade Whirl", function()
    ...
    Menu.NewTree("E", "[E] Wild Rush", function()
    ...
    Menu.NewTree("R", "[R] Inferno Trigger", function()
...
local Combo = function()
    ...
    if rTarget and IsReady(R, function()
    ...
    if rTarget and IsReady(E, function()
        local menuValue = GetMenuValue("ComboUseE")
        local canCast = IsValidTarget(rTarget, R.Range)
        local dist = Player.Position:Distance(rTarget.Position)
        return menuValue and canCast and ((passiveCount >= 4 and IsReady(W)) or passiveCount >= 5 or dist > 300 and IsReady(R))
    end) then
    ...
    if wTarget and IsReady(W, function()
        ...
        return menuValue and canCast and (castMode == 0 and Player.Level >= 6 and passiveCount == 5 or castMode == 1)
    end) then
        return W:Cast()
    end
end
