...
require("common.log")
module("Storm Katarina", package.seeall, log.setup)
clean.module("Storm Katarina", clean.seeall, log.setup)
...
if not (Menu.Get("LastHit.Q") or Q:IsReady()) then return end
   for k,v in pairs (ObjManager.Get("enemy","minions")) do 
        ...
        local Valid =  Q:IsInRange(Minion) and Minion.MaxHealth > 6
        if Valid then 
            local dmg = DmgLib.CalculateMagicalDamage(Player,Minion,dmg(Q,false))
...
function Katarina.CastR(mode)
    ...
    if mode == 1 then 
        for k,rtarget in pairs(GetTargets(R)) do
            local rdmg = DmgLib.CalculateMagicalDamage(Player,rtarget,dmg(Q,true))
...
function Katarina.Auto()
    ...
    if KS(Q) and Q:IsReady() then
        for k,qtarget in pairs(GetTargets(Q)) do
            local dmg = DmgLib.CalculateMagicalDamage(Player,qtarget,dmg(Q,false))
    ...
    if KS(E) and E:IsReady() then
        for k,etarget in pairs(GetTargets(E)) do
            local dmg = DmgLib.CalculateMagicalDamage(Player,etarget,dmg(E,false))
...
function Katarina.LoadMenu()
    Menu.RegisterMenu("StormKatarina", "Storm Katarina", function()
