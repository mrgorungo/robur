...
clean.module("Storm Malphite", clean.seeall, log.setup)
...
function Malphite.OnHighPriority() 
    if not GameIsAvailable() then
        return
    end
    if Malphite.Auto() then return end
    local ModeToExecute = MalphiteHP[Orbwalker.GetMode()]
    if ModeToExecute then
		ModeToExecute()
	end
end

function Malphite.OnNormalPriority()
    if not GameIsAvailable() then
        return
    end
...
function Malphite.LoadMenu()
    Menu.RegisterMenu("StormMalphite", "Storm Malphite", function()
        ...
        Menu.NewTree("Wave", "Wave Clear", function()
        ...
        Menu.NewTree("KillSteal", "KillSteal Options", function()
            Menu.Checkbox("KillSteal.R", "Use [R] to KS", false)     
