...
clean.module("Storm Chogath", clean.seeall, log.setup)
...
function Chogath.OnHighPriority() 
    if not GameIsAvailable() then
        return
    end
    if Chogath.Auto() then return end
    local ModeToExecute = ChogathHP[Orbwalker.GetMode()]
    if ModeToExecute then
		ModeToExecute()
	end
end

function Chogath.OnNormalPriority()
    if not GameIsAvailable() then
        return
    end
...
function Chogath.OnGapclose(source, dash)
    ...
    if spells.Q:IsReady() and  spells.Q:IsInRange(hero) and Menu.Get("Misc.GapQ") then
        if spells.Q:CastOnHitChance(hero,Enums.HitChance.VeryHigh) then return end
    end
    if spells.W:IsReady() and spells.W:IsInRange(hero) and Menu.Get("Misc.GapW") then
        if spells.W:CastOnHitChance(hero,Enums.HitChance.VeryHigh) then return end
    end
end
---@param source AIBaseClient
