  ...
  local foundEnemies = GetHeroes(Player.Position, Viktor.E.Range, "enemy")
   --print(#foundEnemies)
    ...
    if #foundEnemies >=2 then
         ...
         table.sort(foundEnemies,function(a,b)
        ...
        --end7
        if firstTarget.Position:Distance(secTarget.Position)<=650 then --firstTarget and sec target within laserRange
            if Player.Position:Distance(firstTarget.Position) <= ECastRange+50 then --first target within E1 range
            
                 Viktor.E:Cast(secTarget,Player.Position:Extended(firstTarget.Position,Player.Position:Distance(firstTarget.Position)-10))
             
            elseif  Player.Position:Distance(firstTarget.Position) <= Viktor.E.Range then --first target within E2 range
                Pos = Player.Position:Extended(firstTarget.Position, ECastRange)
                possiblePath = Geometry.Rectangle(Pos:Extended(Player.Position,100), secTarget.Position,Lwidth)
                RectStart = Pos:Extended(Player.Position,100)
                RectEnd = secTarget.Position
            --@field Draw fun(self:Rectangle, color:number|nil):nil
                if possiblePath:Contains(firstTarget.Position) then --if 2target will hit
                 
                    pred = Prediction.GetPredictedPosition(Target, Viktor.E2, Pos)
                    if pred and pred.HitChance >=HitChance then
                        Viktor.E:Cast(pred.CastPosition,Pos)                
                    end    
                else --cast only for target
                    startPos = Player.Position:Extended(Target.Position, ECastRange)
                    pred = Prediction.GetPredictedPosition(Target, Viktor.E, startPos)
                    if pred and pred.HitChance >=HitChance then
    ...
    elseif #foundEnemies ==1 then 
            if Player.Position:Distance(Target.Position) <= ECastRange  then   --enemy within ecast range
                Viktor.E:Cast(Target,Player.Position:Extended(Target.Position,Player.Position:Distance(Target.Position)-100))
            end
            if Player.Position:Distance(Target.Position) <= Viktor.E.Range  then --within max range
                    startPos = Player.Position:Extended(Target.Position, ECastRange)
                    pred = Prediction.GetPredictedPosition(Target, Viktor.E2, startPos)
                    if pred and pred.HitChance >=HitChance then
  ...
  Menu.RegisterMenu("Goru Viktor", "Goru Viktor", function ()
    ...
    Menu.Separator()
     Menu.ColumnLayout("Misc", "Misc", 1, true, function ()
      ...
      Menu.Slider("Misc.EHitChance","EHitChance",0.7, 0, 1, 0.01)
...
function Viktor.OnDraw()
  ...
  --[[
  if RectStart and RectEnd then

  Renderer.DrawRectOutline3D(RectStart, RectEnd,Lwidth,1,0xFFFFFFFF)
  end
  ...
  local OrbwalkerLogic = Viktor.Logic[OrbwalkerMode]
   ...
   if Menu.Get("KS.Q") then
    ...
    if Menu.Get("KS.E") then
        e = Viktor.TargetSelector:GetTargets( Viktor.E.Range-70, true)
        for k,v in pairs(e) do 
            local dmg = DamageLib.CalculateMagicalDamage(Player,v,dmg(Viktor.E))
            local Ks  = Viktor.E:GetKillstealHealth(v)
            if dmg > Ks and Viktor.Logic.E(v, 0.9,1) then return end
